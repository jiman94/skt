...
public class Util {
	...
	public static void getHeapSize() {
		...
		logger.info("HeapMemory : \t (u/c/m) "+mf.getUsed()/mb+"MB / "+mf.getCommitted()/mb+"MB / "+mf.getMax()/mb+"MB");

		mf = ManagementFactory.getMemoryMXBean().getNonHeapMemoryUsage();
		logger.info("NonHeapMemory : \t (u/c/m) "+mf.getUsed()/mb+"MB / "+mf.getCommitted()/mb+"MB / "+mf.getMax()/mb+"MB");

		logger.info("Detail Heap Memory : young gen(eden+survivor), old gen(tenured gen), perm gen");
	    for (MemoryPoolMXBean mx : ManagementFactory.getMemoryPoolMXBeans()) {
	    	mf = mx.getUsage();
	    	logger.info(String.format("\t%-20s \t (u/c/m) %15s",mx.getName(), mf.getUsed()/mb+"MB / "+mf.getCommitted()/mb+"MB / "+mf.getMax()/mb+"MB"));
	    }
	    logger.info("-----------------------------------------------");
	}
